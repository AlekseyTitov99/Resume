...
public class MySQL implements SQLConnection {
	...
	public void runUpdate(SQLUpdateRunnable sql, boolean asynchronously) {
		if (asynchronously) {
			new BukkitRunnable() {
	...
	public void runSQL(SQLRunnable sql, boolean asynchronously) {
		if (asynchronously) {
			new BukkitRunnable() {
	...
	public void openConnection() {
		if (connection != null) {
			throw new SQLException("Não foi possível inicializar o MySQL pois esta conexão atual já está aberta.");
		}
		try {
			this.connection = DriverManager.getConnection("jdbc:mysql://"+host+":"+port+"/"+database+"?autoReconnect=true", user, password);
			Bukkit.getConsoleSender().sendMessage("§aA conexão com o MySQL foi efetuada com sucesso, criando tabelas...");
			createTable();
	...
	public void closeConnection() {
		if(connection == null) {
			throw new SQLException(" A conexão remota com o servidor MySQL já foi encerrada.");
		}
		try {
			connection.close();
		} catch (java.sql.SQLException e) {
	...
	public void createTable() {
		if(connection!=null) {
			if(this.tablesNames.size() > 0) {
				...
				throw new SQLException("Nenhuma tabela foi adicionada para ser carregada.");
			}
		}else {
			throw new SQLException("A conexão com o servidor MySQL ainda não foi aberta por este motivo a tabela não foi criada.");
		}
	}
}
