...
public class TimeFormatter {
    ...
    public String format() {
        ...
        if (dias > 0)
            sb.append(dias + (dias == 1 ? " d" : " d"));

        if (horas > 0)
            sb.append(dias > 0 ? (minutos > 0 ? ", " : " e ") : "").append(horas + (horas == 1 ? " h" : " h"));

        if (minutos > 0)
            sb.append(dias > 0 || horas > 0 ? (segundos > 0 ? ", " : " e ") : "").append(minutos + (minutos == 1 ? " m" : " m"));

        if (segundos > 0)
            sb.append(dias > 0 || horas > 0 || minutos > 0 ? " e " : (sb.length() > 0 ? ", " : "")).append(segundos + (segundos == 1 ? " s" : " s"));

        String s = sb.toString();
        return s.isEmpty() ? "0 s" : s;
    }
